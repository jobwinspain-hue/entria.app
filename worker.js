      ...
      try {
        ...
        if (event.type === 'checkout.session.completed') {
          ...
          if (email) {
            ...
            if (userId) {
              // 2. Crear/actualizar perfil Pro en tabla profiles
              await fetch(
              ...
              // 3. Mandar email de bienvenida con credenciales
              await sendWelcomeEmail(email, tempPassword, env);

              // 4. Registrar pago en tabla pagos
              await fetch(
                `${env.SUPABASE_URL}/rest/v1/pagos`,
                {
                  method: 'POST',
                  headers: {
                    'apikey': env.SUPABASE_SERVICE_KEY,
...
// Mandar email de bienvenida con credenciales via Resend o Supabase
async function sendWelcomeEmail(email, password, env) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:32px">
      ...
      <div style="margin-bottom:28px">
        <p style="color:#5a5650;font-size:14px;line-height:1.6;margin:0">⚠️ Te recomendamos cambiar tu contraseña después del primer acceso desde <strong>Ajustes → Cambiar contraseña</strong>.</p>
      </div>
      ...
      <p style="color:#5a5650;font-size:13px;text-align:center;margin-top:24px">¿Alguna duda? Escríbenos a jobwinspain@gmail.com</p>
    </div>
  `;

  // Intentar enviar via Resend si está configurado
  ...
  // Si no hay Resend, el usuario puede recuperar contraseña desde la web
}
